...
const STATUS_COLORS = {
  'Zum Zweitgespräch einladen': { bg: 'rgba(99, 102, 241, 0.12)', text: '#A5B4FC', border: 'rgba(99, 102, 241, 0.3)' },
  'Auf Warteliste': { bg: 'rgba(251, 191, 36, 0.1)', text: '#FCD34D', border: 'rgba(251, 191, 36, 0.25)' },
  'Absage': { bg: 'rgba(248, 113, 113, 0.1)', text: '#F87171', border: 'rgba(248, 113, 113, 0.25)' },
  'Zum Case Interview einladen': { bg: 'rgba(34, 197, 94, 0.1)', text: '#4ADE80', border: 'rgba(34, 197, 94, 0.25)' },
};
    ...
    <div style={{ padding: `${theme.spacing.xl}px`, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.md }}>
        {sorted.map(({ key, data }) => {
          ...
          const statusColor = STATUS_COLORS[status] || { bg: 'rgba(255,255,255,0.04)', text: theme.colors.text.muted, border: theme.colors.border.glass };
          ...
          return (
            <div key={key} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: theme.spacing.md }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: theme.font.sm, color: theme.colors.text.muted, display: 'flex', gap: 12 }}>
                      ...
                      <span style={{ color: theme.colors.text.accent }}>{runde}</span>
                    </div>
                  </div>
                </div>
                  ...
                  {Object.keys(DIMENSIONS).map((dk) => {
                    ...
                    return (
                      <div key={dk} style={{
                        ...
                        padding: '3px 10px', borderRadius: theme.radius.sm,
                        background: `${color}0A`, border: `1px solid ${color}20`,
                        fontSize: theme.font.xs, color: theme.colors.text.secondary,
                      }}>
              ...
              <div style={{ display: 'flex', gap: theme.spacing.sm, flexWrap: 'wrap' }}>
                <button onClick={() => onOpenDetail(data, key)} style={btnStyle}>
                  Detailbericht
                </button>

                {canStartZweit && !isInZweit && (
                  <button onClick={() => handleStartZweit(data)} style={{
                    ...btnStyle,
                ...
                {!isInZweit && !canStartZweit && (
                  <button onClick={() => handleStartZweit(data)} style={btnStyle}>
                ...
                {confirmDelete === key ? (
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    ...
                    <button onClick={() => handleDelete(key)} style={{ ...btnStyle, borderColor: 'rgba(248,113,113,0.4)', color: theme.colors.danger.text }}>
                      Ja
                    </button>
                    <button onClick={() => setConfirmDelete(null)} style={btnStyle}>
                      Nein
                    </button>
                  </div>
                ) : (
                  <button onClick={() => setConfirmDelete(key)} style={{ ...btnStyle, color: theme.colors.text.muted }}>
                    Löschen
                  </button>
                )}
              </div>
